...
当你还不确定应该选择哪种模式时，可以只描述目标：

```text
使用 $pickfolio，帮我完成一个美股投研任务。
先根据我的目标判断应该使用 select、position、full、refresh 还是 review 模式。
只向我确认会实质改变结果的缺失参数，其余采用默认值并记录到 manifest。
遵守选股冻结和仓位审批闸门，不要下单。
```

### 通用参数模板

将方括号中的内容替换成自己的信息；与本次任务无关的参数可以删除：

```text
使用 $pickfolio，研究【美股主题 / 行业 / 候选股票池】，执行【模式或期望结果】。

研究目标：【发现方向 / 完成选股 / 规划仓位 / 更新数据 / Review 已有材料】
输入材料：【已有持仓、候选池、报告、数据目录或 manifest 路径；没有则写“无”】
投资期限：【例如 1-3 年、3-5 年】
已有持仓或禁止重复暴露：【股票代码或“无”】
基准：【例如 SPY；不确定时使用默认值】
资金与币种：【仅 position / full 需要】
最大可承受损失、现金下限和杠杆规则：【仅 position / full 需要】
数据截止日：【latest 或 YYYY-MM-DD】
data_policy：【reuse / refresh-missing / refresh-all】
retain：【minimal / standard / audit】
输出目录：【目标目录；未指定时创建新的不可覆盖运行目录】

缺少会实质改变结论的参数时先询问我；其余参数使用默认值并明确记录。
只生成研究和仓位计划，不执行真实交易。
```

模式既可以直接写成 `select`、`position`、`full`、`refresh` 或 `review`，也可以只描述期望结果，由 Pickfolio 选择合适模式。

### 场景示例

以下示例展示如何针对具体任务补充约束，不是固定的行业、资金或仓位要求。
